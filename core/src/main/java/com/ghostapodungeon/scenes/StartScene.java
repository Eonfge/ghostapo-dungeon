...
import com.ghostapodungeon.windows.WndChallenges;
import com.ghostapodungeon.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		if (!(huntressUnlocked = Badges.isUnlocked( Badges.Badge.BOSS_SLAIN_3 ))) {
			...
			RenderedTextMultiline text = renderMultiline( Messages.get(this, "unlock"), 9 );
		...
		updateClass( HeroClass.values()[ghostapodungeon.lastClass()] );
		...
		Badges.loadingListener = new Callback() {
			...
			public void call() {
				if (Game.scene() == StartScene.this) {
					ghostapodungeon.switchNoFade( StartScene.class );
				}
			}
		};
	}
	...
	private void startNewGame() {
		...
		if (ghostapodungeon.intro()) {
			ghostapodungeon.intro( false );
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( ghostapodungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(ghostapodungeon.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( ghostapodungeon.challenges() > 0 ?
