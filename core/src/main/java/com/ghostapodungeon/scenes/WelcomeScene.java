...
import com.ghostapodungeon.effects.Fireball;
...
public class WelcomeScene extends PixelScene {
	...
	private static int LATEST_UPDATE = ghostapodungeon.v0_6_2;

	@Override
	public void create() {
		super.create();

		final int previousVersion = ghostapodungeon.version();

		if (ghostapodungeon.versionCode == previousVersion) {
		...
		DarkRedButton okay = new DarkRedButton(Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				ghostapodungeon.switchScene(TitleScene.class);
			}
		};
		...
		if (previousVersion != 0){
			DarkRedButton changes = new DarkRedButton(Messages.get(this, "changelist")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					ghostapodungeon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= ghostapodungeon.versionCode) {
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion <= ghostapodungeon.v0_6_0b){
		...
		ghostapodungeon.version(ghostapodungeon.versionCode);
	}
