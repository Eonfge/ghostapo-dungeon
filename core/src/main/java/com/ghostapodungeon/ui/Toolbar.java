...
package com.ghostapodungeon.ui;
...
import com.ghostapodungeon.windows.WndJournal;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = ghostapodungeon.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 152){
				if (width < 138){
					if ((ghostapodungeon.flipToolbar() && i == 3) ||
							(!ghostapodungeon.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !ghostapodungeon.flipToolbar() ||
						i == 3 && ghostapodungeon.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && ghostapodungeon.flipToolbar() ||
							i == 3 && !ghostapodungeon.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(ghostapodungeon.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
		...
		if (ghostapodungeon.flipToolbar()) {
