...
import com.ghostapodungeon.scenes.TitleScene;
...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					ghostapodungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							ghostapodungeon.language(langs.get(langIndex));
			...
			if (ghostapodungeon.landscape() && i % 2 == 1){
		...
		resize(ghostapodungeon.landscape() ? WIDTH_L : WIDTH_P, y);
