...
public class WndTextInput extends Window {
	...
	public WndTextInput(final String title, final String initialValue, final int maxLength,
		...
		final int width;
		if (ghostapodungeon.landscape() && multiLine){
			width = W_LAND_MULTI; //more editing space for landscape users
		} else {
			width = WIDTH;
		}

		ghostapodungeon.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				textInput = new EditText(ghostapodungeon.instance);
				...
				ghostapodungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			ghostapodungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager) ghostapodungeon
					...
					ghostapodungeon.updateSystemUI();
